...
export const PATCH: APIRoute = async ({ params, request }) => {
  ...
  const depositAmount =
    body.depositAmount !== undefined ? parseFloat(body.depositAmount) : undefined;
  ...
  if (manual) {
    ...
    if (depositAmount !== undefined) (manual as any).depositAmount = depositAmount;
    ...
    if (depositAmount !== undefined) (overrides[id!] as any).depositAmount = depositAmount;
  ...
  if (
    guestName &&
    !skipNames.includes(guestName.toLowerCase()) &&
    !guestName.startsWith("Guest ···")
  ) {
    try {
      ...
      console.log(
        `[guests] auto-linked booking ${id} to guest profile ${guestProfile.id} (${guestProfile.fullName})`,
      );
    } catch (err: any) {

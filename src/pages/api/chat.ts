...
import { createExpense, isValidExpenseCategory, readExpenses } from "../../lib/expenses.ts";
import {
  createIncome,
...
function loadChatSession(): { messages: any[]; updatedAt: string } | null {
  try {
    ...
    // Expire after 24h
    if (data.updatedAt && Date.now() - new Date(data.updatedAt).getTime() > CHAT_MAX_AGE)
      return null;
    return data;
  } catch {
    return null;
  ...
  try {
    ...
    const roomLines = rooms
      .map((room) => {
        ...
        const departing = allBookings.find(
          (b) => (b.room === room || b.room === "full") && b.checkout === todayStr && !skip(b),
        );
        const arriving = allBookings.find(
          (b) => (b.room === room || b.room === "full") && b.checkin === todayStr && !skip(b),
        );
        let status = "Empty";
        let guest = "";
        if (arriving && departing) {
          status = "TURNOVER";
          guest = `${departing.guest || "?"} out, ${arriving.guest || "?"} in`;
        } else if (arriving) {
          status = "CHECK-IN";
          ...
          status = "Occupied";
          guest = occupied.guest || "?";
          const left = Math.round(
            (new Date(occupied.checkout).getTime() - today.getTime()) / 86400000,
          );
          guest += ` (${left}d left)`;
        }
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      arrivals.forEach((b) =>
        upcoming.push(`  ${dStr}: ${b.guest || "?"} -> ${labels[b.room] || b.room}`),
      );
    }
...
RECEIPTS AND ACCOUNTING (Kin Haus operators only):
When the user sends a photo or screenshot of a receipt, shop bill, bank notification, Wise payout, or cash note, read the merchant, total amount, and date from the image. If they want it logged (or it is clearly a request to record spending or income), call create_expense or create_income. Do not guess amounts: if unreadable, ask for the total in THB.
- Expenses: use create_expense with vendor (merchant name), amount in THB, date YYYY-MM-DD, category (supplies, food, utilities, maintenance, travel, fees, other), scope business (default) or personal, optional notes.
- Income (Airbnb to Wise, cash from guests, business account): use create_income with description, amount THB, date, category (airbnb, direct_booking, coworking, monitor_rental, transfer, other), deposit_account (wise, cash, business, personal_other, other), scope business or personal.
After logging, confirm the id and totals briefly. For P&L questions use get_accounting_summary. For recent lines use list_recent_accounting.
...
const TOOLS: OpenAI.Chat.Completions.ChatCompletionTool[] = [
  ...
  },
  {
    ...
    function: {
      name: "list_bookings",
      description:
        "List all bookings (Airbnb + manual) with optional date range filter. Returns full booking details including guest, room, dates, type, amount. Amounts include overrides.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get monthly occupancy and revenue stats that match the dashboard sidebar exactly. Use this for any revenue or occupancy question. Defaults to current month.",
      parameters: {
        type: "object",
        properties: {
          month: {
  ...
  },
  {
    ...
    function: {
      name: "list_inquiries",
      description:
        "List all booking inquiries with their status (new, responded, booked, archived).",
      parameters: { type: "object", properties: {} },
    },
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get monthly revenue summary for monitor rentals including active count, total revenue, and utilisation.",
      parameters: { type: "object", properties: {} },
    },
  },
  {
    ...
    function: {
      name: "create_booking",
      description:
        "Create a manual booking. Use this after the user confirms via the [CONFIRM:booking|...] card.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Save or update a guest profile with passport details. Merges with existing profile if guest already exists (matched by passport number or name).",
      parameters: {
        type: "object",
        properties: {
          ...
          nationality: {
            type: "string",
            description: 'Nationality (e.g. "British", "German", "Thai")',
          },
          ...
          booking_ids: {
            type: "array",
            items: { type: "string" },
            description: "Booking IDs to link (optional)",
          },
        },
        required: ["full_name"],
      },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          name: {
            type: "string",
            description: "Guest name to search for (optional - omit to list all)",
          },
        },
      },
    },
  },
  {
    ...
    function: {
      name: "update_booking",
      description:
        "Update an existing manual booking. Use list_bookings to find the booking ID first.",
      parameters: {
        type: "object",
        properties: {
          ...
          tm30_status: {
            type: "string",
            enum: ["pending", "submitted", "not_required"],
            description: "TM30 registration status",
          },
        },
        required: ["id"],
      },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Mark a booking TM30 registration as submitted. Use after completing the TM30 form.",
      parameters: {
        type: "object",
        properties: {
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          type: {
            type: "string",
            enum: ["cleaning", "maintenance", "laundry", "inspection", "other"],
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get a comprehensive daily briefing: room status, arrivals, departures, housekeeping, inquiries, gap alerts, revenue. Use when user asks for morning briefing or daily overview.",
      parameters: { type: "object", properties: {} },
    },
  },
  {
    ...
    function: {
      ...
      description:
        "Get revenue analytics: ADR, RevPAR, occupancy trends, direct vs Airbnb split, gap-night cost estimates. Use for revenue questions or optimization suggestions.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      name: "get_gap_nights",
      description:
        "Find empty nights per room in the next N days with fill suggestions and estimated lost revenue.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      name: "create_expense",
      description:
        "Log an expense from a receipt or operator request. Call after reading amounts from an attached image or explicit text. Amounts are THB.",
      parameters: {
        type: "object",
        properties: {
          date: { type: "string", description: "YYYY-MM-DD" },
          amount: { type: "number", description: "Total in THB" },
          vendor: { type: "string", description: "Merchant or payee name" },
          category: {
            type: "string",
          ...
          scope: {
            ...
            description:
              "business = Kin Haus P&L (default). personal = tracked but excluded from business net.",
          },
          notes: { type: "string", description: "Optional notes" },
        },
  ...
  },
  {
    ...
    function: {
      name: "create_income",
      description:
        "Log income (Airbnb payout to Wise, cash, business bank, etc.) from a screenshot or operator request. Amounts are THB.",
      parameters: {
        type: "object",
        properties: {
          date: { type: "string", description: "YYYY-MM-DD" },
          amount: { type: "number", description: "Amount in THB" },
          description: { type: "string", description: "Short label e.g. Airbnb payout 15 Mar" },
          category: {
            type: "string",
          ...
          deposit_account: {
            ...
            description: "Where the money landed",
          },
          scope: {
            type: "string",
            enum: ["business", "personal"],
            description: "business counts toward Kin Haus net",
          },
          notes: { type: "string", description: "Optional notes" },
        },
        required: ["date", "amount", "description", "category", "deposit_account", "scope"],
      },
    },
  },
  {
    ...
    function: {
      ...
      description:
        "P&L summary: business income vs expenses (net), personal totals, income by deposit account. All THB. Optional date range.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    type: "function",
    function: {
      name: "list_recent_accounting",
      description:
        "List the most recent expense and income lines (default 15 each) for quick reference.",
      parameters: {
        type: "object",
        properties: {
          limit: { type: "number", description: "Max rows per type (default 15, max 40)" },
        },
      },
    },
  },
  ...
  for (const room of rooms) {
    for (let d = 1; d <= numDays; d++) {
      ...
      const bk = allBookings.find(
        (b) => (b.room === room || b.room === "full") && b.checkin <= dStr && b.checkout > dStr,
      );
      if (bk) {
        if (bk.type === "waitlist") {
          waitlistNights++;
  ...
  switch (name) {
    ...
    case "create_booking": {
      try {
        ...
        return {
          success: true,
          booking: newBooking,
          message: `Booking created for ${newBooking.guest} in ${newBooking.room} (${newBooking.checkin} to ${newBooking.checkout})`,
        };
      } catch (err: any) {
    ...
    case "save_guest_profile": {
      try {
        ...
        return {
          success: true,
          guest: profile,
          message: `Guest profile saved for ${profile.fullName}`,
        };
      } catch (err: any) {
    ...
    case "update_booking": {
      try {
        ...
        if (args.payment_status !== undefined)
          (bookings[idx] as any).paymentStatus = args.payment_status;
        if (args.tm30_status !== undefined) (bookings[idx] as any).tm30Status = args.tm30_status;
        writeManualBookings(bookings);
        return {
          success: true,
          booking: bookings[idx],
          message: `Booking updated for ${bookings[idx].guest}`,
        };
      } catch (err: any) {
    ...
    case "mark_tm30_submitted": {
      try {
        ...
        return {
          success: true,
          message: `TM30 marked as submitted for booking ${args.booking_id}`,
        };
      } catch (err: any) {
    ...
    case "delete_booking": {
      try {
        ...
        const idx = bookings.findIndex((b) => b.id === args.id);
        if (idx === -1)
          return { error: "Booking not found or is an Airbnb booking (cannot delete)" };
        const deleted = bookings.splice(idx, 1)[0];
        writeManualBookings(bookings);
        return {
          success: true,
          message: `Deleted booking for ${deleted.guest} (${deleted.room}, ${deleted.checkin} to ${deleted.checkout})`,
        };
      } catch (err: any) {
    ...
    case "update_inquiry": {
      try {
        ...
        return {
          success: true,
          message: `Inquiry from ${inquiries[idx].guest} marked as "${args.status}"`,
        };
      } catch (err: any) {
    ...
    case "create_housekeeping_task": {
      try {
        ...
        return {
          success: true,
          task,
          message: `Housekeeping task created: ${args.title} for ${args.room} on ${args.date}`,
        };
      } catch (err: any) {
      ...
      try {
        ...
        const roomStatus = rooms.map((room) => {
          ...
          return {
            room: labels[room],
            status,
            guest: occupied?.guest || arriving?.guest || departing?.guest || null,
            daysLeft: occupied
              ? Math.round(
                  (new Date(occupied.checkout).getTime() - new Date(todayStr).getTime()) / 86400000,
                )
              : null,
          };
        });
        ...
        const inHouse = allBookings.filter(
          (b) => todayStr >= b.checkin && todayStr < b.checkout && !skip(b),
        );
        ...
        const guestTm30 = inHouse.map((b) => {
          const g = b.guestId
            ? guests.find((gg) => gg.id === b.guestId)
            : guests.find((gg) => gg.fullName?.toLowerCase() === b.guest?.toLowerCase());
          const hasTm30 = g && g.nationality && g.passportNumber && g.dateOfBirth;
          return {
            guest: b.guest,
            room: labels[b.room] || b.room,
            tm30Ready: !!hasTm30,
            missing: !hasTm30
              ? [
                  !g?.nationality && "nationality",
                  !g?.passportNumber && "passport",
                  !g?.dateOfBirth && "DOB",
                ].filter(Boolean)
              : [],
          };
        ...
        return {
          ...
          inHouse: guestTm30,
          housekeeping: {
            pending: todayHk.filter((t) => t.status === "pending").length,
            inProgress: todayHk.filter((t) => t.status === "in_progress").length,
            done: todayHk.filter((t) => t.status === "done").length,
          },
          inquiries: { new: newInq.length, total: inquiries.length },
          monthToDate: {
            occupancy: stats.occupancyPercent + "%",
            revenue: stats.estimatedRevenue + " THB",
          },
        };
      } catch (err: any) {
          ...
          channelSplit: {
            ...
            directPercent:
              recentBookings.length > 0
                ? Math.round((directCount / recentBookings.length) * 100)
                : 0,
          },
          recommendation:
            adr > 0
              ? `ADR is ${adr} THB. RevPAR is ${revpar} THB. ${directCount > airbnbCount ? "Direct bookings are leading -- great margin." : "Most bookings via Airbnb -- consider promoting direct bookings for better margins."}`
    ...
    case "create_expense": {
      try {
        ...
        if (!category || !isValidExpenseCategory(String(category))) {
          return { error: `Invalid category. Use one of: ${VALID_EXPENSE_CATEGORIES.join(", ")}` };
        }
        const scope = args.scope === "personal" ? "personal" : "business";
        const result = createExpense({
          ...
          category,
          scope,
          notes: args.notes != null ? String(args.notes) : "",
          source: "dashboard",
        });
        ...
        return {
          ...
          message: `Saved expense ${result.id}: ${result.vendor} ${result.amount} THB (${result.date})`,
        };
      } catch (err: any) {
        return { error: err.message || "Could not save expense" };
      }
    }
    ...
    case "create_income": {
      try {
        const category = args.category;
        const depositAccount = args.deposit_account;
        const scopeStr = args.scope;
        if (!category || !isValidIncomeCategory(String(category))) {
          return { error: `Invalid category. Use one of: ${VALID_INCOME_CATEGORIES.join(", ")}` };
        }
        if (!depositAccount || !isValidDepositAccount(String(depositAccount))) {
          return {
            error: `Invalid deposit_account. Use one of: wise, cash, business, personal_other, other`,
          };
        }
        if (!scopeStr || !isValidIncomeScope(String(scopeStr))) {
          return { error: "scope must be business or personal" };
        }
        const result = createIncome({
          ...
          category,
          depositAccount,
          scope: scopeStr === "personal" ? "personal" : "business",
          notes: args.notes != null ? String(args.notes) : "",
          source: "dashboard",
        });
        if ("error" in result) return { error: result.error };
        return {
          ...
          message: `Saved income ${result.id}: ${result.description} ${result.amount} THB (${result.date})`,
        };
      } catch (err: any) {
        return { error: err.message || "Could not save income" };
      }
    }
    ...
    case "get_accounting_summary": {
      try {
        ...
        return { error: err.message || "Could not build summary" };
      }
    }

    case "list_recent_accounting": {
      try {
        ...
        const incomes = [...readIncomes()]
          ...
          .map((i) => ({
            ...
            category: i.category,
            depositAccount: i.depositAccount,
            scope: i.scope,
          }));
        return { expenses, incomes };
      } catch (err: any) {
        return { error: err.message || "Could not list accounting" };
      }
    }
    ...
    case "get_gap_nights": {
      try {
        ...
        return {
          ...
          suggestion:
            totalEmpty > 5
              ? `${totalEmpty} empty nights could generate up to ${totalLost} THB. Consider last-minute deals or reaching out to past guests.`
              : totalEmpty > 0
                ? `Only ${totalEmpty} gap night(s) -- looking good.`
                : "Fully booked! No gaps.",
        };
      } catch (err: any) {
...
export const GET: APIRoute = async () => {
  ...
  return new Response(
    JSON.stringify({ messages: session?.messages || [], updatedAt: session?.updatedAt || null }),
    {
      headers: { "Content-Type": "application/json" },
    },
  );

...
export function findGuestByName(name: string): GuestProfile | undefined {
  ...
  return (
    guests.find((g) => g.fullName.toLowerCase() === lower) ||
    guests.find(
      (g) => g.fullName.toLowerCase().includes(lower) || lower.includes(g.fullName.toLowerCase()),
    )
  );
}
...
export function saveGuestProfile(
  profile: Partial<GuestProfile> & { fullName: string },
): GuestProfile {
  const guests = readGuests();
 ...
 */
export function syncGuestStats(
  guestId: string,
  bookings: { id: string; checkin: string; checkout: string; amount: number; type: string }[],
): void {
  const guests = readGuests();
